...
Graph: `new` (`--obs` seeds at creation), `obs`, `link`, `rm`, `rm-obs`, `unlink`, `graph`, `search` (`--where key=value` truth filters; query term optional), `show`. Truths: `truth`, `rm-truth`. Plus `skills`, `ingest`, `query` (`--json`/`--limit`), `compact`, `init`, `backup`, `restore`, `stats`, `export` (`--scope <entity>` for a single-epic subgraph bundle, `--rationale` for the decision chain), `import`, `reset`, and `schema [--command NAME]`. Machine-readable responses use the versioned `{schemaVersion, ok, data|error}` envelope; see [`docs/response-contract.md`](docs/response-contract.md). Full reference: [`SKILL.md`](SKILL.md), [`docs/usage.md`](docs/usage.md).
