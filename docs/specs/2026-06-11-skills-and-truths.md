...
3. **Skills subsystem** — `rosemary skills install|update|remove` plus `rosemary skills`
...
```bash
P="harus-nix"
rosemary add-truth "$P:session" objective    "cleanup pass — …"
rosemary add-truth "$P:session" status       "DONE"
rosemary add-truth "$P:session" completed    "dropped mise-tasks…; committed 7c28260"
rosemary add-truth "$P:session" remaining    "none — working tree clean…"
rosemary add-truth "$P:session" next         "none pending"
rosemary add-truth "$P:session" last-updated "2026-06-11"
```
...
No DB or MCP change. `rosemary add-observations foo "a" "b" "c"` adds three rows in one
transaction.
...
- `rosemary add-truth <entity> <key> <value>` — upsert one truth.
- `rosemary delete-truth <entity> <key>` — remove one truth.
...
- Downstream: the rosemary **skill** (`SKILL.md` workflows) and the **MCP tool schemas**
...
```
rosemary open-nodes "skill:jasonswett-llm-skills:writing-tests"
```

returns the entity's truths + the body (eager). Discovery is `rosemary skills` or
`search-nodes`.
...
- **When `documents` is enabled** (additive, feature-gated): also chunk + embed skill
  bodies into the existing vector store so `rosemary query` finds skills semantically.
...
- `documents` feature: skill bodies are queryable via `rosemary query`.
