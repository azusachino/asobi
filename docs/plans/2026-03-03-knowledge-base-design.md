...
2. **Core CLI:** Implement the basic command structure using `clap`.
3. **Storage Engines:** Initialize libSQL schemas and LanceDB tables.
4. **Ingestion Pipeline:** Build the logic to sync Markdown files with the databases.
5. **Vector Search:** Integrate an embedding model (e.g., via `candle` or a local API) for LanceDB.
