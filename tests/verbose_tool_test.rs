use asobi::db::init_db;
use asobi::mcp::handle_tools_call;
use serde_json::json;
...
async fn test_create_entities_returns_graph_state() {
    ...
    unsafe {
        std::env::set_var(asobi::db::ENV_DATABASE_URL, db_path.to_str().unwrap());
    }
    ...
    let graph: asobi::mcp::Graph = serde_json::from_str(result_content).unwrap();
