...
- **`purge` finds expired superseded truth versions**, grouped by entity and key and ranked by volume, and drops them with `--history`. Truth history is the one store with no bound of its own: observations are capped per entity and current truths are one row per key, but every overwrite appends a version that survives until its entity is deleted — so it grows fastest on the entity written most often. On a six-week-old graph that is 575 versions, 394 of them on a single session entity whose `next` and `remaining` are rewritten at every closeout. They are always listed in the preview, since finding what expired is read-only; deleting requires the flag. Superseded values are safe to drop by construction — the current value lives in a different table and is never touched — and the cutoff is inclusive, unlike the entity one, because a version is dead the moment it is replaced.
