...
- **The skill provenance manifest moved out of the skills directory and into the data directory.** It was written as `.asobi-skills.json` beside the installed skills; it is now `skills.json` under `data_dir` — `.asobi/data/` project-local, `~/.local/share/asobi/data/` under XDG.
  ...
  It records the skills directory it describes, in a top-level `dir` field, because the two paths are not anchored together: under the XDG fallback `data_dir` is global while the skills directory follows the working directory, so one data directory can be reached from several skills trees. `read_installed_skills` now falls back to scanning the actual directory whenever the manifest is absent, unreadable, **or names a different tree** — previously it returned early on any manifest that parsed, so a mismatch surfaced as an empty list, which is indistinguishable from an empty tree and left `update` reporting "no skills with a recorded source" over a populated directory.
