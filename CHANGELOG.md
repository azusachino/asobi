...
- **Bundled skill resources are installed.** A skill that owns a directory now brings `references/`, `scripts/`, `assets/` and anything else it ships along with its body. Previously only `SKILL.md` was written, so a conformant skill bundling `scripts/extract.py` installed "successfully" and was silently broken — the body still said to run the script and the file was not there. Resources are refreshed on every sync, since an upstream change to a reference file does not necessarily change `SKILL.md`. Symlinks are not followed: a skill source is untrusted input.
...
- **`make check` validates installed skills against the Agent Skills specification**, using the reference validator published alongside the spec. One divergence is deliberate and documented: the spec requires a skill's directory name to equal its frontmatter `name`, which assumes single-source authoring, while Asobi installs many sources into one tree as `<source-slug>@<skill-name>` — two sources may ship the same name, and agent hosts surface the directory name as the skill's identity. Each skill's content is validated under a conformant name, so everything else the spec requires is enforced.
...
- **`export` carries the truth change trail.** Snapshots gain a `truthHistory` block covering the exported entities, so a scoped handoff — the documented way to give an epic to another agent — can distinguish a fact that was always true from one corrected an hour ago. The field is defaulted rather than versioned, so an older snapshot imports as an empty history and an older reader ignores it; re-importing does not duplicate the trail.
