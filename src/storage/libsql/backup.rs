...
use crate::storage::libsql::constant::{
...
/// otherwise the libSQL provider's default state file inside the resolved
/// workspace.
pub fn effective_db_path() -> PathBuf {
    std::env::var(ENV_DATABASE_URL)
        ...
        .unwrap_or_else(|_| {
            AsobiPaths::resolve()
                .data_dir
                .join(crate::storage::libsql::db::DEFAULT_DATABASE_FILENAME)
        })
}
...
fn default_backup_path(db_path: &Path, prefix: &str) -> PathBuf {
    let stamp = chrono::Local::now().format("%Y%m%d-%H%M%S%3f");
    backups_dir(db_path).join(format!("{prefix}-{stamp}.db"))
}
...
pub async fn backup(
    ...
    let dest = backup_to(conn, db_path, output).await?;
    if is_default {
        prune_backups(db_path, BACKUP_PREFIX, keep);
    }
    Ok(dest)
}
...
async fn backup_to(conn: &Connection, db_path: &Path, output: Option<PathBuf>) -> Result<PathBuf> {
    let dest = output.unwrap_or_else(|| default_backup_path(db_path, BACKUP_PREFIX));
    if dest.exists() {
...
pub async fn restore(
    ...
    let safety = backup_to(
        &conn,
        db_path,
        Some(default_backup_path(db_path, PRE_RESTORE_PREFIX)),
    )
    .await
...
///
/// libSQL keeps a rebuildable internal vector index whose physical ordering can
/// change during `VACUUM INTO`. It is an index, not graph data, so tolerate only
/// notices that name that known internal index.
...
fn prune_backups(db_path: &Path, prefix: &str, keep: usize) {
...
mod tests {
    ...
    async fn seed(conn: &Connection, name: &str) {
        crate::storage::libsql::db::create_entities(
            conn,
            vec![EntityInput {
                name: name.to_string(),
    ...
    async fn backup_then_restore_roundtrip() {
        ...
        let (db, conn) = crate::storage::libsql::db::init_db_at(&db_file)
            .await
            .unwrap();
        seed(&conn, "alpha").await;

        backup(&conn, &db_file, Some(snap.clone()), 3)
            .await
            .unwrap();
        assert!(snap.exists());

        // Mutate after the backup so a successful restore is observable.
        crate::storage::libsql::db::reset(&conn).await.unwrap();

        restore(db, conn, &db_file, &snap, true).await.unwrap();

        let (_db, conn) = crate::storage::libsql::tx::open_local(&db_file)
            .await
            .unwrap();
        let graph = crate::storage::libsql::db::open_nodes(&conn, vec!["alpha".to_string()])
            .await
            .unwrap();
    ...
    async fn backup_refuses_existing_output() {
        ...
        let snap = dir.path().join("snap.db");
        let (_db, conn) = crate::storage::libsql::db::init_db_at(&db_file)
            .await
            .unwrap();
        backup(&conn, &db_file, Some(snap.clone()), 3)
            .await
        ...
        let err = backup(&conn, &db_file, Some(snap), 3).await.unwrap_err();
    ...
    async fn restore_rejects_non_asobi_file() {
        ...
        let (db, conn) = crate::storage::libsql::db::init_db_at(&db_file)
            .await
            .unwrap();
        let err = restore(db, conn, &db_file, &bogus, true).await.unwrap_err();
    ...
    async fn default_backup_prunes_to_keep() {
        ...
        backup(&conn, &db_file, None, 2).await.unwrap();
