...
pub fn effective_db_path() -> PathBuf {
    std::env::var(ENV_DATABASE_URL)
        .map(PathBuf::from)
        .unwrap_or_else(|_| RosemaryPaths::resolve().db_path())
}

/// Directory holding managed snapshots, co-located with the live DB so backups
/// stay next to the data they snapshot even under `ROSEMARY_DATABASE_URL`.
...
pub async fn restore(db: Database, conn: Connection, source: &Path, force: bool) -> Result<()> {
    ...
    validate_snapshot(source)
        .await
        .with_context(|| format!("{} is not a valid Rosemary database", source.display()))?;
...
/// Open `source` and confirm it carries the expected Rosemary schema and passes
            ...
            .open(&lock_path)
        {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => bail!(
                "another backup/restore is in progress (lock: {}). \
                 If no rosemary process is running, delete that file and retry.",
                lock_path.display()
            ),
...
mod tests {
    ...
    #[tokio::test]
    async fn restore_rejects_non_rosemary_file() {
        let dir = tempdir().unwrap();
        ...
        assert!(err.to_string().contains("not a valid Rosemary database"));
    }
    ...
    async fn default_backup_prunes_to_keep() {
        ...
        for name in [
            "rosemary-20200101-000000000.db",
            "rosemary-20200102-000000000.db",
        ] {
        ...
        let mut remaining: Vec<String> = std::fs::read_dir(&bdir)
            ...
            .filter(|n| n.starts_with("rosemary-"))
            .collect();
        remaining.sort();
        ...
        assert!(
            remaining
                .iter()
                .any(|n| n.as_str() > "rosemary-20200102-000000000.db")
        );
    }
