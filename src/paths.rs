...
impl RosemaryPaths {
    pub fn resolve() -> Self {
        // 1. Try project-local config.toml first
    ...
    fn from_config(conf: RosemaryConfig, root: &Path) -> Self {
        Self {
            ...
            kb_dir: conf.kb_dir.unwrap_or_else(|| root.join(".rosemary/topics")),
        }
    }
