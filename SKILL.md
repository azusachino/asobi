...
Ingests a file or directory of Markdown files into the KB (chunks, embeds, stores in LanceDB). Used for the semantic search tier.

```
rosemary query <QUERY>
```

Hybrid semantic + FTS keyword search over ingested KB topics. Returns: `TITLE | (score: X.XX) | PATH` per result.
...
Controlled by `rosemary.toml` in the project root (takes precedence over XDG paths):
